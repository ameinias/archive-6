...
function HashImport() {
  ...
  const { globalUser, setStatusMessage } = GameLogic();
  ...
  const importHash = async () => {
    ...
    if (badHashes.includes(hashValue)) {
      setStatusMessage("[HEXHASH] BAD GATEWAY \n Irreconcilable corruption present");
    ...
    for (const subItem of foundSubItems) {
      ...
      if (!parentID || isNaN(Number(parentID))) {
        console.warn(
          `Invalid parentID for subitem ${subItem.id}:`,
          subItem.parentId,
        );
    ...
    <div className="hashImport-div">
      <div className="row">
        ...
        <div className="instructions ">
          <p>
            Migrate corrupted entries to the new database by entering hexhashes
            here.
          </p>
          <p>
            Your coordinator or administrator will have provided these hexes
            either as a print out or a spreadsheet.{" "}
          </p>
        </div>
        {/*// ------ Title  ------*/}
        <div className="row">
