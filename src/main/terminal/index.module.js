...
async function main() {
    try {
        await gaiman.load(`https://cdn.jsdelivr.net/npm/figlet/lib/figlet.js`)
        const $_figlet = figlet;
        const $_Array = Array;
        await $_figlet.defaults({ [`fontPath`]: `https://unpkg.com/figlet/fonts/` });
        await $_figlet.preloadFonts(await $_Array.from(new GaimanArray(`Standard`)), $_ready);
        let $_service = await gaiman.rpc(`https://terminal.jcubic.pl/service.php`);
        async function $_ready() {
            let $_ascii = await $_figlet.textSync(`Jargon File`, { [`font`]: `Standard` });
            gaiman.echo(`<yellow>${ $_ascii }</yellow>`);
            await gaiman.echo_extra(`<white>This is the Jargon File, a comprehensive compendium of hacker slang illuminating many aspects of hackish tradition, folklore, and humor.</white>`, 20);
            gaiman.echo();
            while (true) {
                if (gaiman.should_break_loop(2)) {
                    break;
                }
                let $_term = await gaiman.ask(`jargon? `);
                for (let $_result of await $_service.jargon($_term)) {
